...
class MLPNet(nn.Module):
    ...
    def forward(self, world_pts, target_viewdirs, latent, style_latent):
	    ...
	    # For RGB branch
            ...
            hyper_rgb = self.hyper_rgb(style_latent)
            base_remap = hyper_rgb[0](x4)
            rgb0 = hyper_rgb[1](torch.cat((base_remap, target_viewdirs), dim=-1))
            rgb1 = hyper_rgb[2](rgb0)

	    # For RGB branch
            rgb2 = hyper_rgb[3](rgb1)
            rgb3 = hyper_rgb[4](rgb2)
            rgb = self.rgb_sigmoid(rgb3)
...
class NerfNet(nn.Module):
    ...
    def forward(self, ray_o, ray_d, fg_z_max, fg_z_vals, bg_z_vals, latent, style_latent):
        ...
        fg_raw = self.fg_net(input_pts, input_viewdirs, latent, style_latent)
